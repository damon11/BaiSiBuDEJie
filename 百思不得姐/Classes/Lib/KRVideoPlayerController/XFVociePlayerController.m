...
- (void)updateProgressInfo
{
    ...
    NSInteger minute = self.player.currentPlaybackTime / 60;
    NSInteger second = (NSInteger)self.player.currentPlaybackTime % 60;
    self.restTime.text = [NSString stringWithFormat:@"%02zd:%02zd",minute,second];
    
    // 2.更新滑块的位置
    self.progress.value = self.player.currentPlaybackTime / self.player.duration;
    if(self.progressTimer == 0){
        [self dismiss];
...
- (IBAction)sliderValueChange {
    // 设置当前播放的时间Label
    NSInteger minute = self.player.duration * self.progress.value / 60;
    NSInteger second = (NSInteger)(self.player.duration * self.progress.value) % 60;
    self.restTime.text = [NSString stringWithFormat:@"%02zd:%02zd",minute,second];
